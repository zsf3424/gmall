...
public class SearchServiceImpl implements SearchService {
    ...
    private SearchResponse parseResult(SearchResult searchResult) {
        ...
        if (!CollectionUtils.isEmpty(brandAggBuckets)) {
            ...
            brandVo.setName("品牌");
            List<String> brandValues = brandAggBuckets.stream().map(brandAggBucket -> {
        ...
        if (!CollectionUtils.isEmpty(categoryAggBuckets)) {
            ...
            List<String> categroyValues = categoryAggBuckets.stream().map(categoryAggBucket -> {
                HashMap<String, Object> map = new HashMap<>();
            ...
            categoryVo.setValue(categroyValues);
        ...
        List<SearchResponseAttrVO> attrVOS = attrIdAgg.getBuckets().stream().map(bucket -> {
            ...
            List<String> values = attrValueAgg.getBuckets().stream().map(valueBucket -> valueBucket.getKeyAsString()).collect(Collectors.toList());
        ...
        // 解析商品列表的结果集
        List<GoodsVO> goodsVOS = searchResult.getSourceAsObjectList(GoodsVO.class, false);
